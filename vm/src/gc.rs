...
// The only way we can deallocate something that we didnt mean to is if we have a LivaPointer somewhere other than the stack, globals, or in a reachable HeapObj
// So be careful if we ever alloc a LivaClosure or a LivaInstance when a LivaPointer is floating around on the rust stack
...
impl GC {
        ...
        match obj_opt {
            Some(obj) => {
                if obj.is_marked == false {
                    ...
                    self.grey_worklist.push(index); // Only the values that are pointed to by LivaPointers (instances and closures) can contain values that might be garbage collected
                }
            }
    ...
    fn mark_value(&mut self, val: &Value) {
        if let Value::LivaPointer(ptr) = val {
    ...
    fn mark_grey(&mut self) {
        while !self.grey_worklist.is_empty() {
            ...
            match obj_opt {
                Some(obj) => {
                    // Blacken -> Look for LivaPointers that might be stored in these HeapObjs
                    if DEBUG_GC {
                        eprintln!("blackening {:?} at {}", obj.obj_type, index)
                    }
                    match &obj.obj {
                        HeapObjVal::LivaClosure(closure) => {
                        ...
                        HeapObjVal::LivaInstance(instance) => {
