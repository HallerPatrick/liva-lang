...
/// Manages the declaration and definiton of local variables
///
///
/// A `ResolverNode` is created and pushed onto the stack.
/// It gets popped off once its definition ends, so the value on top of each `ResolverNode`
...
/// A `ResolverNode` is created for each function definition
#[derive(Debug, Clone)]
struct ResolverNode {
